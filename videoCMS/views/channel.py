...
def index(request):
    ...
    for one in channelList:
        ...
        one['sourceSearchList'] = zip(
            map(lambda a: urllib.unquote(a.encode('utf-8')).decode('utf-8'), one['sourceList']),
            one['searchHandleList'])
...
def update(request):
    ...
    if img:
        ...
        #更新所有的resourceImage
        clct_resource.update({'channelId': channel['channelId'], 'resourceImageUrl': ''},
                             {'$set': {'resourceImageUrl': channel['resourceImageUrl']}}, multi=True)

    #更新资源的 channelId关联
    oldChannel = clct_channel.find_one({'_id': ObjectId(id)})

    if channel['channelId'] != oldChannel['channelId']:
        clct_resource.update({'channelId': oldChannel['channelId']}, {'$set': {'channelId': channel['channelId']}},
                             multi=True)
...
def add(request):
    ...
    if img:
        ...
        #更新到所有
        clct_resource.update({'channelId': channel['channelId'], 'resourceImageUrl': ''},
                             {'$set': {'resourceImageUrl': filename}}, multi=True)
...
def detail(request):
    ...
    channel['detailTrailerList'] = [
        {"url": trailer[3], "title": trailer[2], "videoType": trailer[0], "videoId": trailer[1]}
...
def pushChannel(request):
    ...
    if cronTime == "":
        ...
        task = {"type": "AndroidPush", "pushChannelId": channelId, "pushTitle": title, "pushContent": content,
                "extras": extras}
