...
def POST2Resource(request):
    ...
    resource['tagList'] = [tag for tag in
                           map(lambda a: a.strip(), request.POST.get('tagList').replace(u'，', ',').split(',')) if
                           tag != '']
...
def sendReviewFailMessage(_from, resource, reason):
    ...
    msg = {'from': _from, 'to': resource['editor'], 'title': '审核失败', 'content': content, 'createTime': getCurTime(),
           'type': 'reviewFail',
